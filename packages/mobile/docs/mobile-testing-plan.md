...
- [ ] Add `packages/mobile/app/src/test/kotlin/de/chennemann/opencode/mobile/data/NetworkServiceTest.kt` with fakes or Robolectric to verify connectivity state and `changed` increments
- [ ] Add `packages/mobile/app/src/test/kotlin/de/chennemann/opencode/mobile/data/MdnsEntryTest.kt` for URL formatting and IPv6 normalization, and add `MdnsService` instrumentation coverage only if testability hooks are introduced
